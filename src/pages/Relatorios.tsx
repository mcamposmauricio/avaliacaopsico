  ...
  const generateReport = useMutation({
    mutationFn: async ({ campaignId, type }: { campaignId: string; type: string }) => {
      const { error } = await supabase.from("reports").insert({
        ...
        report_type: type,
        tenant_id: tenantId,
        file_url: null, // Will be generated by edge function
      });
      if (error) throw error;
    },
    onSuccess: () => {
              ...
              {campaigns.map((c: any) => (
                <div key={c.id} className="flex items-center justify-between p-3 border rounded-lg">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => generateReport.mutate({ campaignId: c.id, type: "technical" })}>
                      <FilePlus className="h-4 w-4 mr-1" />Laudo Técnico
                    </Button>
                    <Button size="sm" variant="outline" onClick={() => generateReport.mutate({ campaignId: c.id, type: "executive" })}>
                      <FilePlus className="h-4 w-4 mr-1" />Rel. Executivo
                    </Button>
                  </div>
                </div>
              ))}
            ...
            <TableBody>
              {reports.length === 0 ? (
                ...
                <TableRow key={r.id}>
                  ...
                  <TableCell>
                    <Button variant="ghost" size="icon" disabled={!r.file_url}>
